...
public class MainLab2Advanced {

    public static void main(String[] args) {
        final var labId = parseLabId(args);
        ...
        if (labId <= 2) {
            runFrontEndForCase(
                    FilePathConfig.SRC_CODE_PATH,
                    FilePathConfig.TOKEN_PATH,
                    FilePathConfig.OLD_SYMBOL_TABLE,
                    FilePathConfig.PARSER_PATH
            );
            runFrontEndForCase(
                    FilePathConfig.SRC_CODE_MOREFUNC_PATH,
                    FilePathConfig.TOKEN_MOREFUNC_PATH,
                    FilePathConfig.OLD_SYMBOL_TABLE_MOREFUNC,
                    FilePathConfig.PARSER_MOREFUNC_PATH
            );
            return;
        }
    ...
    private static int parseLabId(String[] args) {
        if (args.length == 0) {
    ...
    private static void runFrontEndForCase(
            String sourcePath,
            String tokenPath,
            String oldSymbolTablePath,
            String parserPath
    ) {
        final var symbolTable = new SymbolTable();

        final var lexer = new LexicalAnalyzer(symbolTable);
        lexer.loadFile(sourcePath);
        lexer.run();
        lexer.dumpTokens(tokenPath);
        final var tokens = lexer.getTokens();
        symbolTable.dumpTable(oldSymbolTablePath);
        ...
        // 各 Observer 输出结果
        productionCollector.dumpToFile(parserPath);
    }
}